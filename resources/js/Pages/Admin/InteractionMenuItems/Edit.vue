...
const form = useForm({
    ...
    parent_id: props.item.parent_id ? String(props.item.parent_id) : '',
    order_column: String(props.item.order_column ?? 0),
...
const submit = () => {
    form.put(route('admin.interaction-menu-items.update', props.item.id));
};
</script>

<template>
    <Head :title="`Редактирование: ${props.item.title}`" />

    <AuthenticatedLayout>
        <template #header>
            <div class="flex items-center justify-between gap-4">
                <h2 class="font-semibold text-xl text-gray-800 leading-tight">Редактировать пункт меню</h2>
